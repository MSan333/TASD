...
OUTPUT_DIR = Path("/tmp/rank_mini")
OUTPUT_DIR.mkdir(exist_ok=True)

TARGET_TOTAL = 10000

# ─── 完整 System Prompt（从训练日志中提取） ─────────────────────────────
...
def build_prompt(gt):
    """构建完整 prompt（与训练日志中的格式一致）"""
    ctx = gt['context']

    # 判断大促
    campaign_data_str = ctx.get('campaign_data', '{}')
    try:
        campaign_data = json.loads(campaign_data_str) if isinstance(campaign_data_str, str) else campaign_data_str
    except:
        campaign_data = {}
...
{ctx.get('campaign_data', '{}')}"""
    ...
    # 构建 messages 格式的 prompt（verl 支持的格式）
    prompt = [
...
print(f"\n每站点目标: {per_venture} (+余数 {remainder})")

sampled = []
for i, (venture, count) in enumerate(ventures.items()):
    venture_df = all_data[all_data['venture'] == venture]
    target = per_venture + (1 if i < remainder else 0)

    if len(venture_df) <= target:
        sampled.append(venture_df)
        actual = len(venture_df)
    else:
        # 优先保留有 negative 的样本
        has_neg = venture_df[venture_df['ground_truth'].apply(
            lambda x: len(json.loads(x).get('negative_keys', [])) > 0
        )]
        no_neg = venture_df[venture_df['ground_truth'].apply(
            lambda x: len(json.loads(x).get('negative_keys', [])) == 0
        )]

        neg_sampled = has_neg  # 全部保留
        neg_count = len(neg_sampled)

        if neg_count >= target:
            sampled.append(has_neg.sample(target, random_state=42))
            actual = target
        else:
            bc_needed = target - neg_count
            bc_sampled = no_neg.sample(min(bc_needed, len(no_neg)), random_state=42)
            sampled.append(pd.concat([neg_sampled, bc_sampled]))
            actual = neg_count + len(bc_sampled)

    print(f"  {venture}: {count} → {actual}")
...
for name, path in [("训练集", train_path), ("验证集", test_path)]:
    ...
    print(f"  positive_actions: 非空={sum(1 for x in pos_lens if x>0)} ({sum(1 for x in pos_lens if x>0)/len(pos_lens)*100:.1f}%), avg={np.mean(pos_lens):.1f}")
    print(f"  negative_keys:    非空={sum(1 for x in neg_lens if x>0)} ({sum(1 for x in neg_lens if x>0)/len(neg_lens)*100:.1f}%), avg={np.mean(neg_lens):.1f}")
    print(f"  card_pool:        avg={np.mean(pool_lens):.1f}")
    print(f"  venture:          {dict(sorted(Counter(ventures).items()))}")

    # 验证 positive 在 pool 中
    ...
    print(f"  prompt 格式: messages={all(isinstance(p, (list, np.ndarray)) for p in prompts)}")
...
files = [
    (str(train_path), "ad/guoshauile.gsl/data/minirank/train.parquet"),
    (str(test_path), "ad/guoshauile.gsl/data/minirank/test.parquet"),
]

for local_path, oss_path in files:
    size_mb = Path(local_path).stat().st_size / 1024 / 1024
    print(f"  Uploading {local_path} ({size_mb:.1f} MB) -> oss://{BUCKET_NAME}/{oss_path}")
    result = bucket.put_object_from_file(oss_path, local_path)
    print(f"  Status: {result.status} {'OK' if result.status == 200 else 'FAILED'}")
